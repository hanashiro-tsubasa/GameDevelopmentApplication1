...
Scene::Scene() : objects(), back_ground(NULL), EnemyNumber(), WingEnemyNumber(), GoldEnemyNumber(), HarpyNumber(), GameScore(),Gametime(),fream_count()
{
...
{
	//プレイヤーを画面中央あたりに生成する
	CreateObject<Player>(Vector2D(320.0f, 60.0f),0.0f);
	//背景の読み込み
	back_ground = LoadGraph("Resource/Images/BackGround.png");
	BGM=LoadSoundMem("sounds/BGM_arrows.wav");
...
void Scene::Update()
{
	...
	if (Gametime > 0)
	{
		...
		{
			CreateObject<Enemy>(Vector2D(-40.0f, 400.0f), 0.0f);
			EnemyNumber += 1;
		}
		if (Createrand == 2 && EnemyNumber < 6)//ハコテキの一度の生成上限数) /*(InputControl::GetKeyDown(KEY_INPUT_Z))*/
		{
			CreateObject<Enemy>(Vector2D(679.0f, 400.0f), 0.0f);
			EnemyNumber += 1;
		}
		/////////////////////////////////////////////
		...
		{
			CreateObject<WingEnemy>(Vector2D(-40.0f, 150.0f + rand() % 150), 0.0f);
			WingEnemyNumber += 1;
		}
		if (Createrand == 4 && WingEnemyNumber < 6)//ハネテキの一度の生成上限数) /*(InputControl::GetKeyDown(KEY_INPUT_Z))*/
		{
			CreateObject<WingEnemy>(Vector2D(679.0f, 150.0f + rand() % 150), 0.0f);
			WingEnemyNumber += 1;
		}
		/////////////////////////////////////////////
		...
		{
			CreateObject<GoldEnemy>(Vector2D(-40.0f, 420.0f), 0.0f);
			GoldEnemyNumber += 1;
		}
		if (GoldCreaterand == 6 && GoldEnemyNumber < 1)//金テキの一度の生成上限数) /*(InputControl::GetKeyDown(KEY_INPUT_Z))*/
		{
			CreateObject<GoldEnemy>(Vector2D(679.0f, 420.0f), 0.0f);
			GoldEnemyNumber += 1;
		}
		/////////////////////////////////////////////
		...
		{
			CreateObject<Harpy>(Vector2D(-40.0f, 150.0f + rand() % 150), 0.0f);
			HarpyNumber += 1;
		}
		if (HarpyCreaterand == 8 && HarpyNumber < 2)//ハーピーの一度の生成上限数) /*(InputControl::GetKeyDown(KEY_INPUT_Z))*/
		{
			CreateObject<Harpy>(Vector2D(679.0f, 150.0f + rand() % 150), 0.0f);
			HarpyNumber += 1;
		}
		/////////////////////////////////////////////
		...
		{
			CreateObject<Bomb>(Vector2D(objects[0]->GetLocation().x, objects[0]->GetLocation().y + 70.0f), 0.0f);
		}
		////////////////////////////////////////////
		...
		for (int i = 0; i < objects.size(); i++)
		{
			...
			if (objects[i]->GetShotFlg() == TRUE)
			{
				...
				//objects[0]->GetLocation();
				float v =
					sqrtf((objects[0]->GetLocation().x - objects[i]->GetLocation().x) * (objects[0]->GetLocation().x - objects[i]->GetLocation().x) +
						(objects[0]->GetLocation().y - objects[i]->GetLocation().y) * (objects[0]->GetLocation().y - objects[i]->GetLocation().y));
				float bulletradian_x = (objects[0]->GetLocation().x - objects[i]->GetLocation().x) / v;
				float bulletradian_y = (objects[0]->GetLocation().y - objects[i]->GetLocation().y) / v;

				CreateObject<EnemyBullet>(Vector2D(objects[i]->GetLocation()), Vector2D(bulletradian_x, bulletradian_y));
...
void Scene::Draw()const
{
	...
	DrawFormatString(300, 455, 0xFFFFFF, "%d", GameScore);
	DrawFormatString(500, 455, 0xFFFFFF, "%d", GameScore);
	DrawFormatString(60, 455, 0xFFFFFF, "%d", Gametime);
...
void Scene::HitCheckObject(GameObject* a, GameObject* b)
{
	...
	if ((fabsf(diff.x) < box_size.x) && (fabsf(diff.y) < box_size.y))
	{
		...
		a->OnHitCollision(b);
		b->OnHitCollision(a);
		if (a->GetSerialNumber() == 3)
		{
			//GameScore += a->GetScore();
			GameScore += b->GetScore();
		}
		else if (b->GetSerialNumber() == 3)
		{
			//GameScore += a->GetScore();
			GameScore += a->GetScore();
		}
